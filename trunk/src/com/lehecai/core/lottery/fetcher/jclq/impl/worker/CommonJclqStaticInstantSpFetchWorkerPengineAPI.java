...
public class CommonJclqStaticInstantSpFetchWorkerPengineAPI extends AbstractJclqStaticInstantSpFetchWorker{
	...
	public List<JclqStaticInstantSpItem> fetchJclqSp(String officialDate, LotteryType lotteryType){
		...
		if (list == null || list.size() == 0) {
			logger.error("竞彩篮球抓取官方固定奖金即时sp失败");
			return null;
		}
		String matches = list.get(0);
		if (matches == null || "".equals(matches)) {
			logger.error("未获取到竞彩篮球官方固定奖金即时sp");
			return null;
		}
