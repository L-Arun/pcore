 ...
 *
 */
public class FootballScheduleFetch10WorkerOfficial extends AbstractFootballScheduleFetchWorker{
	protected final Logger logger = LoggerFactory.getLogger(this.getClass().getName());
	
	protected final static String BQC_SCHEDULE_FETCH_URL_OFFICIAL = "http://www.lottery.gov.cn/lottery/bqc/BQC.aspx";
	...
	public List<FootballScheduleItem> fetchFootballSchedule(String phase) {
		...
		try{
			...
			for (int i = 1; i < rows.length; i=i+2) {
				...
				String[] teams = tableColumns[3].toPlainTextString().replace("　", "").replace(" ", "").replace("&nbsp;", "").split("vs");
				footballScheduleItem.setHomeTeam(teams[0]);
				footballScheduleItem.setAwayTeam(teams[1]);
