...
/**
 * 胜负彩 足球赛程 Okooo 抓取worker
 * 2010-3-30
 *
 */
public class FootballScheduleFetch7WorkerOkooo extends AbstractFootballScheduleFetchWorker {
		protected final Logger logger = LoggerFactory.getLogger(this.getClass().getName());
		
		protected final static String SFC_SCHEDULE_FETCH_URL_OKOOO = "http://www.okooo.com/zucai/";
		...
		public List<FootballScheduleItem> fetchFootballSchedule(String phase) {
			...
			url = url + "/" + this.getFetchPhase();
			data = CoreFetcherUtils.URLPost(url, null, encoding);
			...
			try {
				...
				for(int i=2;i<16;i++){
					...
					TableColumn[] tableColumn = tableRows[i].getColumns();
					//场次
					footballScheduleItem.setMatchIndex(Integer.parseInt(tableColumn[0].toPlainTextString()));
					//主队
					footballScheduleItem.setHomeTeam(CoreStringUtils.trimAll(tableColumn[1].getChildren().elementAt(0).toPlainTextString()));
					//客队
					footballScheduleItem.setAwayTeam(CoreStringUtils.trimAll(tableColumn[3].getChildren().elementAt(0).toPlainTextString()));
					//联赛或赛事名称
					footballScheduleItem.setLeague(CoreStringUtils.trimAll(tableColumn[4].getChildren().elementAt(0).toPlainTextString()));
					//比赛时间
					String strDate = tableColumn[4].getChildren().elementAt(2).toPlainTextString().trim();
		...
		public String getScheduleUrl() {
			...
			return SFC_SCHEDULE_FETCH_URL_OKOOO;
		}
	}
