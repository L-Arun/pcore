 ...
 *
 */
public class LotteryDrawFetch554WorkerOfficial extends AbstractLotteryDrawFetchWorker {
	...
	protected String lotteryScope = "好运彩3";
	
	public LotteryDrawFetch554WorkerOfficial() {
		super(LotteryType.A_YZFCHYC3);
	}
		...
		try {
			...
			for (int i = 2; i < tableRows.length; i++) {
				...
				if (phaseTmp != null && !"".equals(phaseTmp) && phase.equals(phaseTmp)) {
					...
					lotteryDraw.setResult(strResult[0] + "," + strResult[1] + "," + strResult[2]);
					//彩种
		...
		try {
			...
			while (it.hasNext()) {
				...
				while (childIt.hasNext()) {
					...
					if (childElement.getName().equals("drawSaleCount3")) {
