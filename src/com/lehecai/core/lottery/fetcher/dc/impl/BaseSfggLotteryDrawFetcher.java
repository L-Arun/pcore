...
import com.lehecai.core.lottery.fetcher.dc.ISfggLotteryDrawFetcher;
import com.lehecai.core.util.CoreObjectUtils;

/**
 * 基础胜负过关赛程开奖结果抓取，抽象，待具体实现
 *
 */
public abstract class BaseSfggLotteryDrawFetcher implements ISfggLotteryDrawFetcher {
	...
	public List<SfggLotteryDrawItem> fetch(String phase, FetcherType fetcherType) throws UnsupportedFetcherTypeException{
		...
		try {
			return (List<SfggLotteryDrawItem>) m.invoke(this, o);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			return null;
		}
