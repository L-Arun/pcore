...
/**
 * 6场半 足球赛程 500WanWeb 抓取worker
 * @author liurd
 *
 */
public class FootballScheduleFetch10Worker500WanWeb extends AbstractFootballScheduleFetchWorker{
	...
	protected final static String BQC_SCHEDULE_FETCH_URL_500WAN = "http://zc.trade.500wan.com/zc6/index.php";
	...
	public List<FootballScheduleItem> fetchFootballSchedule(String phase) {
		...
		try{
			if (data.indexOf("id=\"vsTable\"") != -1) {
				data = data.substring(data.indexOf("id=\"vsTable\""), data.indexOf("id=\"all_form\">"));
				data = "<html><head></head><body><table " + data + "></div></body>";
			}
			...
			for (int i = 0 ; i < tableRows.length - 1; i=i+2) {
				...
				if (tableColumns[3].toPlainTextString().indexOf("[") != -1) {
					teams = tableColumns[3].toPlainTextString().substring(4, tableColumns[3].toPlainTextString().length() - 4).split("VS");
				} else {
					teams = tableColumns[3].toPlainTextString().split("VS");
				}
				footballScheduleItem.setHomeTeam(CoreStringUtils.trimAll(teams[0]));
				footballScheduleItem.setAwayTeam(CoreStringUtils.trimAll(teams[1]));
	...
	public String getScheduleUrl() {
		...
		if (phase != null && !"".equals(phase)) {
			link = BQC_SCHEDULE_FETCH_URL_500WAN + "?expect=" + phase;
			return link;
		}
		...
		try {
			...
			link = BQC_SCHEDULE_FETCH_URL_500WAN + "?expect=" + getFetchPhase();
