 ...
 *
 */
public class FootballScheduleFetch10WorkerClient extends AbstractFootballScheduleFetchWorker {
	...
	public List<FootballScheduleItem> fetchFootballSchedule(String phase) {
		...
		List<String> result;
		try {
			result = CoreHttpUtils.postUrl(this.getScheduleUrl(), "action=get6cMatch&phase=" + phase, CharsetConstant.CHARSET_UTF8, QueueConstant.HTTP_TIME_OUT_DEFAULT);
		} catch (IOException e) {
