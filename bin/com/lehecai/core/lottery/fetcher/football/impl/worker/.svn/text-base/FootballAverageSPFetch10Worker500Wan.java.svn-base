...
/**
 * 6场半全场  平均欧赔  500Wan 抓取worker
 * @author 唐容
 *
 */
public class FootballAverageSPFetch10Worker500Wan extends
	...
	protected String AverageSP_URL_500Wan = "http://trade.500wan.com/static/public/zc6/daigou/shtml/%s.shtml";
		...
		try {
			...
			if( nodeList != null && nodeList.size() > 0){
				...
				while(itr.hasMoreNodes()){
					...
					CompositeTag ulTag = (CompositeTag) itr.nextNode();
					if( ulTag.getAttribute("class")!=null && ulTag.getAttribute("class").indexOf("iframe_tr") >= 0){
						...
						Bullet liTag = (Bullet) liList.elementAt(0);
						Div div = (Div) liTag.getChild(1);
						String mathIndexStr = div.toPlainTextString().trim();
						item.setMatchIndex(Integer.parseInt(mathIndexStr));		//比赛场次
				
						liTag = (Bullet) liList.elementAt(4);
						parser.setInputHTML(liTag.toHtml());
						NodeList divList = parser.extractAllNodesThatMatch(new TagNameFilter("div"));
						
						String averageSPStr = divList.elementAt(0).toPlainTextString().trim();		//半场-平均欧指
						String[] averageSPArr = averageSPStr.split(spaceMark);
						if (averageSPArr != null && averageSPArr.length == 3) {
							item.setAverageSP_BC_S(subSp(averageSPArr[0]));
							item.setAverageSP_BC_P(averageSPArr[1]);
							item.setAverageSP_BC_F(averageSPArr[2]);
						} else {
							item.setAverageSP_BC_S("--");
							item.setAverageSP_BC_P("--");
							item.setAverageSP_BC_F("--");
						}
						
						averageSPStr = divList.elementAt(1).toPlainTextString().trim();				//全场-平均欧指
						averageSPArr = averageSPStr.split(spaceMark);
						if (averageSPArr != null && averageSPArr.length == 3) {
							item.setAverageSP_S(subSp(averageSPArr[0]));
	...
	private String subSp(String tempSp) {
		if (tempSp != null && !"".equals(tempSp) && tempSp.indexOf(":") != -1) {
			tempSp = tempSp.substring(tempSp.indexOf(":")+1);
		}
		return tempSp;
	}
