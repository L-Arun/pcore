...
/**
 * 6场半全场 足球赛程 Okooo 抓取worker
 * 2010-3-30
 *
 */
public class FootballScheduleFetch10WorkerOkooo extends AbstractFootballScheduleFetchWorker {
		protected final Logger logger = LoggerFactory.getLogger(this.getClass().getName());
		
		protected final static String JQC_SCHEDULE_FETCH_URL_OKOOO = "http://www.okooo.com/zucai/liuban/";
		...
		public List<FootballScheduleItem> fetchFootballSchedule(String phase) {
			...
			try {
				...
				for(int i=2,j=1;i<14;i=i+2,j++){
					...
					//主队
					footballScheduleItem.setHomeTeam(CoreStringUtils.trimAll(tableColumn[1].getChildren().elementAt(0).toPlainTextString().trim()));
					//客队
					footballScheduleItem.setAwayTeam(CoreStringUtils.trimAll(tableColumn[3].getChildren().elementAt(0).toPlainTextString().trim()));
					//联赛或赛事名称
					footballScheduleItem.setLeague(CoreStringUtils.trimAll(tableColumn[5].getChildren().elementAt(0).toPlainTextString()));
					//比赛时间
					String strDate = tableColumn[5].getChildren().elementAt(2).toPlainTextString().trim();
