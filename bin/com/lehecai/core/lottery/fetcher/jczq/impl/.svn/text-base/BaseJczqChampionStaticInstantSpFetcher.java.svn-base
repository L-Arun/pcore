 ...
 *
 */
public abstract class BaseJczqChampionStaticInstantSpFetcher implements IJczqChampionStaticInstantSpFetcher {
	...
	public List<JczqChampionStaticInstantSpItem> fetch(String phase, FetcherType fetcherType, LotteryType lotteryType) throws UnsupportedFetcherTypeException{
		...
		try {
			return (List<JczqChampionStaticInstantSpItem>) m.invoke(this, o);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			return null;
		}
